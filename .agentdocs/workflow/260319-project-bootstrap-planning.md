...
- `【中文译文】` 现在采用双层兜底：
  - 先尝试结构化 JSON 翻译
  - 失败后自动切换到纯文本翻译
  - 若整段翻译仍不稳定，再退到分句翻译并聚合结果
- 学术导读模式的状态面板需要显式展示三步动作：
  - `translate_section`
  - `summarize_key_points`
  - `build_glossary`
- 禁止再使用“一轮混合输出 + 事后修补”来同时生成译文、要点和术语，这会导致模块之间互相污染。
- 学术导读结果缓存版本需要随输出契约变化同步提升，避免旧缓存污染新渲染结果。
- 跨页 / 跨列正文合并后，`PaperSection.block_orders` 必须保持完整映射，保证左侧热区点击仍能回到同一个自然段。
  - `ruff check app.py src tests`
  - `ruff format --check app.py src tests`
  - `pytest`
  - `streamlit run app.py --server.headless true`
...
- 已为“论文-代码对齐”主线接入更强的代码感知工具：
  - `requirements.txt` 新增 `jedi`
  - 当前环境已确认 `jedi` 可用
  - Jedi 缓存目录已显式收口到仓库内 `.tmp/jedi-cache`，规避系统目录权限问题
- 已将代码证据从“纯行切片”升级为“AST 逻辑块”：
  - Python 源文件会优先按 `class / function / method` 拆成完整逻辑块
  - 每个 `CodeEvidence` 现在携带 `absolute_path`、`source_content`、`symbol_name`、`block_type`、`docstring`
  - `read_code_segment` 已升级为返回带符号名、逻辑块类型与 Docstring 的完整上下文
- 已为 Agent 增加跨文件追踪能力：
  - 新增 `find_definition` 动作
  - Agent 在遇到不确定函数或类时，可以从调用点追到定义源头
  - 跨文件追踪成功后，会把目标定义块并入候选证据链
- 已强化 Final Answer 的多维验证：
  - 强制输出“证据等级”
  - 强制输出“算子核对”
  - 强制输出“维度核对”
  - 输出重心改为“论文这一段如何由某个类 / 函数实现”，不再回退到变量映射表
- 已同步增强工作区右栏：
  - 结果区会额外展示证据等级、算子核对和维度核对
  - 逻辑对齐与实现链路分开展示，便于阅读
- 已补齐本轮回归测试：
  - AST 逻辑块读取测试
  - Jedi 跨文件定义跳转测试
  - 全量 `pytest` 通过
  - Streamlit 冒烟启动返回 `200`

## 当前章节预审与静默触发修正

- 已在 `PlanAndExecuteAgent.run()` 入口增加“章节语义预审”：
  - `Introduction / Abstract / Conclusion / Related Work / Background` 等片段优先进入“学术导读模式”
  - `Method / Implementation / Architecture / Model / Algorithm` 等片段才允许进入“代码深度对齐模式”
  - 默认策略偏保守：宁可先导读，也不对非技术段落做代码硬匹配
- 已新增“优雅拒绝”分支：
  - 当方法段缺少可靠代码证据时，Agent 会明确说明“当前代码库中无直接对应的算子实现”
  - 不再拿泛化类定义或弱相关模块硬凑结论
  - 若证据不足但存在邻近上下文，也只保留为人工核对线索
- 已按章节语义动态调整 Agent 角色提示：
  - 引言类片段更偏“科研领路人”
  - 方法类片段更偏“代码审计专家”
  - Final Answer 强制优先解释算法思想如何落地，而不是变量映射
- 已修复工作区默认首段触发问题：
  - `selected_section_index` 初始值改为 `None`
  - 首次进入工作区时右侧保持静默，只显示“请选择论文片段开始深度对齐”
  - 章节选择器新增显式占位项，避免页面一加载就自动跑 Agent
- 本轮已补齐回归测试并通过：
  - 引言片段会切到学术导读模式
  - 方法段在证据不足时会拒绝硬凑实现
  - UI 允许初始未选择片段
  - `ruff check` / `ruff format --check` / `pytest` 全部通过

## 当前导师式输出重构

- 已将右侧结果区从“内部诊断面板”改为“科研学习助手”输出：
  - 统一使用 `【中文译文】`
  - 统一使用 `【核心要点】`
  - 统一使用 `【术语百科】`
  - 只有在强关联命中源码时才展示 `【源码落地】`
- 已收起中间推理噪音：
  - 推理进行中仍通过 `st.status` 给出必要反馈
  - 推理完成后自动清空中间 Thought / Action / Observation 展示
  - 不再默认把内部推理链、证据等级、自我审计暴露给用户
- 已重写 Final Answer 与学术导读提示词：
  - 先翻译，再拆解，再解释术语
  - 禁止变量映射表、日志腔和 JSON 痕迹式表达
  - 代码证据不够强时，隐藏源码落地模块，只保留论文讲解
- 已补充界面与输出约束测试：
  - 强命中时才显示源码落地
  - 初始未选中片段时右侧静默
  - 全量 `pytest` 通过
...
- 已修复“结构化对象原样显示到结果区”的问题：
  - 若模型把字典或列表字符串塞进 `【中文译文】/【核心要点】/【术语百科】`，会先做二次清洗
  - 对清洗后仍像半成品的内容，会自动再走一轮“科研学习助手”修复提示
  - 页面不再直接暴露 `{'paper_title': ...}` 这类原始结构化内容
- 已升级对齐缓存版本，避免旧缓存继续污染新结果展示
- 已收敛左侧布局：
  - 移除主视觉里的大号章节下拉框
  - 改为只显示当前聚焦片段的轻量焦点条
  - 用户通过 PDF 页图点击切换阅读焦点，减少表单感
- 本轮本地校验：
  - `ruff check app.py src tests`
  - `ruff format --check app.py src tests`
  - `pytest` 全量通过
...
- 已确认点击页图时传给 Agent 的不是图片像素，而是 PDF 预解析后的 `PaperSection` 文本块
- 已修复导致“半截伪翻译”的两个关键源头：
  - `PDFParser` 现在会把跨行断词和段内换行还原得更接近自然段
  - `EvidenceBuilder.build_focus_sections()` 现在会按位置关系合并相邻正文块，避免用户点到的只是碎片化小块
- 已移除旧的截断式翻译兜底：
  - 不再把论文内容截成 160 个字符后做半中半英替换
  - 当模型结果不稳定时，至少保留完整原段落文本，后续再交给学习助手修复器重写
- 已再次提升对齐缓存版本，避免旧结果继续污染新渲染
- 本轮新增回归测试：
  - PDF 多行断词还原测试
  - 相邻正文块合并为自然段测试
  - 全量 `pytest` 通过
...
- 已扩展 `PaperSection`，新增 `block_orders` 用于记录一个自然段由哪些原始 PDF block 合并而来
- 已增强 `EvidenceBuilder.build_focus_sections()` 的段落合并策略：
  - 同页同列时，按行距和左边界连续性合并
  - 跨页时，如果上一页块位于页尾、下一页块位于页首，且文本看起来仍在续写，则继续合并
  - 跨列时，如果左栏页尾与右栏页首呈现连续语义，则继续合并为一个自然段
- 已修复左侧点击映射：
  - 工作区不再只认 `section.order`
  - 当用户点击一个已合并自然段中的任意原始 block，都会命中同一个 `PaperSection`
- 已再次提升缓存版本到 `learning-output-v6`，避免旧段落切分结果污染当前阅读体验
- 本轮新增回归测试：
  - 跨页正文块合并测试
  - 跨列正文块合并测试
  - 合并后多块点击映射测试
...
- 已修复学习助手对“伪译文”的判定漏洞：
  - 当模型只返回 `:`、空字符串或近似空内容时，不再直接当成合法译文展示
  - 这类结果现在会强制触发一次学习助手重修流程
- 已提升缓存版本到 `learning-output-v7`，避免旧的坏译文继续被页面缓存复用
- 已新增回归测试：
  - 当模型首轮只返回冒号译文时，系统必须修复成正常中文说明
- 本轮本地校验：
  - `ruff check app.py src tests`
  - `ruff format --check app.py src tests`
  - `pytest`
